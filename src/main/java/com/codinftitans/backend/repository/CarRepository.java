...
public interface CarRepository extends JpaRepository<Car, UUID> {
 Page<Car> findAll(Pageable pageable);
    ...
    @Query(value = "select * from car where model ilike %:query% or color ilike %:query% or motor_type ilike %:query% or" +
            " price = CAST(:query AS double precision)",nativeQuery = true)
